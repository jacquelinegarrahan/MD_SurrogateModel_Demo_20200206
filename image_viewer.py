...
source = ColumnDataSource({'image':[img], 'x':[ext[0]] , 'y':[ext[2]], 'dw':[ext[1]-ext[0]] , 'dh':[ext[3]-ext[2]]})
...
p = figure(tooltips=[("x", "$x"), ("y", "$y"), ("value", "@image")], height=400,width=400)
p.x_range.range_padding = p.y_range.range_padding = 0

img_obj = p.image(name='img',image='image', x='x', y='y', dw='dw', dh='dh', source=source, palette=pal)
...
def update():
    ...
    img_obj.data_source.data.update({'image':[img], 'x':[ext[0]] , 'y':[ext[2]], 'dw':[ext[1]-ext[0]] , 'dh':[ext[3]-ext[2]]})
